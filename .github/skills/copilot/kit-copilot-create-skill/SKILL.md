...
4. **Invocation**: Should it appear as a `/` slash command, or only trigger automatically? (determines `user-invokable` and `disable-model-invocation`)
...
- [ ] `argument-hint` is present and helpful (if `user-invokable: true`)
