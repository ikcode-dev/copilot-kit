...
| [create-commit-message-instructions](.github/prompts/create-commit-message-instructions.prompt.md) | Generate custom instructions for Copilot's commit message generation in VS Code's Source Control panel. Defaults to **Conventional Commits** but supports custom formats (Jira tickets, emojis, simple imperative). Outputs ready-to-use `settings.json` config. | When you want consistent, team-aligned commit messages generated by Copilot (✨ sparkle icon in Source Control) | Claude Opus 4.5 Preview (preferred), GPT-5.2 Preview |
